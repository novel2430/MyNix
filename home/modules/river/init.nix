{pkgs, ...}:
{
  home.file.".config/river/init" = {
    text = with pkgs; ''
      ...
      riverctl map normal Super Return spawn ${foot}/bin/foot
      ...
      do
          ...
          riverctl map $mode None XF86AudioRaiseVolume  spawn 'my-volume up'
          riverctl map $mode None XF86AudioLowerVolume  spawn 'my-volume down'
          riverctl map $mode None XF86AudioMute         spawn 'my-volume mute'
          ...
          riverctl map $mode None XF86MonBrightnessUp   spawn '${brightnessctl}/bin/brightnessctl set +10%'
          riverctl map $mode None XF86MonBrightnessDown spawn '${brightnessctl}/bin/brightnessctl set 10%-'

          # ScreenShot
          riverctl map $mode None Print spawn 'grim-slurp-screenshot full'
          riverctl map $mode Super Print spawn 'grim-slurp-screenshot select'

      done

      # Set background and border color
      riverctl background-color 0x002b36
      riverctl border-color-focused 0xd8dee9
      ...
      riverctl map normal Super+Shift M send-layout-cmd rivercarro "main-location left"

      # Set the default layout generator to be rivercarro and start it.
      # River will send the process group of the init executable SIGTERM on exit.
      riverctl default-layout rivercarro
      rivercarro -no-smart-gaps -outer-gaps 20 -inner-gaps 10 &
      ...
      ## Clipboard
      riverctl map normal Super C spawn "wlroot-clipboard"
      ## Lock Screen
      riverctl map normal Super+Shift L spawn "my-swaylock manual"
      ## Power Menu
      riverctl map normal Super+Shift P spawn "wofi-power-menu"
