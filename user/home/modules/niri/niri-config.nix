  ...
  home.file.".config/niri/config.kdl" = {
    ...
    layout {
        ...
        focus-ring {
            ...
            active-color "#${opt-config.colors.focus-border}"
            // active-gradient from="#5e81ac" to="#eceff4" angle=315

            // Color of the ring on inactive monitors.
            //
            // The focus ring only draws around the active window, so the only place
            // where you can see its inactive-color is on other monitors.
            inactive-color "#${opt-config.colors.unfocus-border}"
        ...
        border {
            ...
            urgent-color "#${opt-config.colors.red}"
    ...
    spawn-at-startup "my-niri-autostart"
    ...
    binds {
        ...
        Mod+Shift+C hotkey-overlay-title="Clash Config Switcher" { spawn-sh "clash-switcher"; }
        Mod+Shift+F hotkey-overlay-title="Web Browser: Zen" { spawn-sh "${apps.web-browser}"; }

        XF86AudioRaiseVolume cooldown-ms=150 allow-when-locked=true { spawn-sh "my-volume up"; }
        XF86AudioLowerVolume cooldown-ms=150 allow-when-locked=true { spawn-sh "my-volume down"; }
        XF86AudioMute        allow-when-locked=true { spawn-sh "my-volume mute"; }
        XF86AudioPlay        allow-when-locked=true { spawn-sh "${apps.playerctl} play-pause"; }
        XF86AudioStop        allow-when-locked=true { spawn-sh "${apps.playerctl} stop"; }
        XF86AudioPrev        allow-when-locked=true { spawn-sh "${apps.playerctl} previous"; }
        XF86AudioNext        allow-when-locked=true { spawn-sh "${apps.playerctl} next"; }
        XF86MonBrightnessUp cooldown-ms=150 allow-when-locked=true { spawn "${apps.brightnessctl}" "--class=backlight" "set" "+10%"; }
        XF86MonBrightnessDown cooldown-ms=150 allow-when-locked=true { spawn "${apps.brightnessctl}" "--class=backlight" "set" "10%-"; }
