{ inputs, ... }:
{
  xdg.mimeApps = {
    ...
    # 可选但很推荐：把某些“乱声明 MIME”的程序踢出候选/默认
    associations.removed = {
      "application/zip" = [ "org.ppsspp.PPSSPP.desktop" ];
      "application/x-zip-compressed" = [ "org.ppsspp.PPSSPP.desktop" ];
    };
    ...
    associations.added = {
      ...
      # ✅ 修正：application/zip（不是 applications/zip）
      "application/zip" = [ "org.gnome.FileRoller.desktop" "xarchiver.desktop" ];
      # ✅ 兼容：一些应用会用这个 MIME
      "application/x-zip-compressed" = [ "org.gnome.FileRoller.desktop" "xarchiver.desktop" ];
      ...
      # 可选：有的地方会用 x-gzip
      "application/x-gzip" = [ "org.gnome.FileRoller.desktop" "xarchiver.desktop" ];
    };
