...
let
  ...
  formatGaps = binding:(
    if binding.smart-gap == true then
      "rivercarro -outer-gaps ${binding.outer} -inner-gaps ${binding.inner} -main-ratio 0.5 -per-tag &\n"
    else
      "rivercarro -no-smart-gaps -outer-gaps ${binding.outer} -inner-gaps ${binding.inner} -main-ratio 0.5 -per-tag &\n"
  );
